...
def handle_menu_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    style = MultiMsgStyle([
        MessageStyle(offset=0, length=msg_length, style="color", color="cdd6f4", auto_format=False),
        MessageStyle(offset=0, length=msg_length, style="font", size="14", auto_format=False),
